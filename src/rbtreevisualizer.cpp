...
#include "ui_rbtreevisualizer.h"
#include "mainwindow.h"

RBTreeVisualizer::RBTreeVisualizer(QWidget *parent) :
                                                      QWidget(parent),
                                                      ui(new Ui::RBTreeVisualizer)
{
  ui->setupUi(this);
  pen.setWidth(2);
}
...
void RBTreeVisualizer::paintEvent(QPaintEvent *event) {
...
void RBTreeVisualizer::mouseMoveEvent(QMouseEvent *event) {
...
void RBTreeVisualizer::mousePressEvent(QMouseEvent *event) {
  prev_drag.setX(0);
  prev_drag.setY(0);
...
void RBTreeVisualizer::wheelEvent(QWheelEvent *event) {
  float new_zoom = zoom;
...
void RBTreeVisualizer::VisualizeTree(QPainter *painter, RB::Node *node) {
...
QRect RBTreeVisualizer::BoxForNode(RB::Node *node) {
...
bool RBTreeVisualizer::CheckDeletion(QPoint pos, RB::Node *node) {
  if (node == tree.TNULL) return false;
